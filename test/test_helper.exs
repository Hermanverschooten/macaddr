...
defmodule Macaddr.TestHelper do
  def conn_options do
    db_options = [
      sync_connect: true,
      hostname: "localhost",
...
db_options = Keyword.merge(Macaddr.TestHelper.conn_options, [database: "postgres"])
{:ok, pid} = Postgrex.Connection.start_link(db_options)

Postgrex.Connection.query!(pid, "DROP DATABASE IF EXISTS macaddr_test;", [])
Postgrex.Connection.query!(pid, "CREATE DATABASE macaddr_test;", [])
